# Task Tracer (Task Tracker)

Jednoduchý task tracker postavený na **React (Vite)** + **Node.js/Express (TypeScript)** + **Redis Stack**. Projekt podporuje **role** (student/teacher), **JWT autentizaci**, real‑time aktualizace přes **Socket.IO** a historii notifikací přes **Redis Streams**.
...
- [Spuštění přes Docker Compose](#spuštění-přes-docker-compose)
...
Zastavení (včetně smazání dat ve volume):
...
- Původní `README.md` obsahuje TODO položky (seed script, screenshoty, diagram). Tento `README2.md` popisuje aktuální implementaci podle zdrojového kódu.
